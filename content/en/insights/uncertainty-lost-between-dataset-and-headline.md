...
The interesting part is what gets deleted. It is rarely only the error bar. It is usually a **condition**: the probability the estimate is attached to, the period it averages over, or the baseline it is measured against. Strip the condition and the number does not become less precise. It becomes an answer to a different question.
...
The instrument end is equally careful in its own idiom. NOAA's Global Monitoring Laboratory notes on its global CO₂ record that "the last year of data are still preliminary, pending recalibrations of reference gases and other quality control checks" — a warning that the most recent and most quoted value is the one most likely to move.
...
## Case three: a rate that depends entirely on the window
