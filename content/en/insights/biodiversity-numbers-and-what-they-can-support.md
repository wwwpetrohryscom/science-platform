...
The critiques concede more than their framing suggests. The randomisation study puts its own correction at 9.6% and states in the same sentence that substantial declines are confirmed. The clustering analysis reports three taxonomic–geographic systems declining strongly with high certainty and seven more with less, and proposes a replacement indicator rather than the absence of a signal. Even the pipeline critique, which goes furthest, argues for better measures rather than for the conclusion that vertebrate populations are not changing.
...
None of this makes the indices worthless, and none of it supports the reading that biodiversity trends are unknown. It supports something narrower: each of these numbers has a sentence it can carry, and the sentence is usually shorter and stranger than the one written for it.

For the headline figure the defensible version is roughly this. Across 34,836 monitored vertebrate populations, the average population trend since 1970 corresponds to a 73% reduction in index value, with the steepest measured declines in freshwater systems and in Latin America and the Caribbean, and with the caveat that the estimate is sensitive to a small number of extreme decliners and to how short and zero-containing time series are handled. That is a mouthful, it is longer than a headline tolerates, and every clause in it is doing work that the shorter version silently discards. The version that fails — "the world has lost 73% of its wildlife" — fails on all three counts at once: it converts a rate into a stock, a sample into a world, and a contested estimate into a fact.

The same failure recurs wherever a derived quantity gets a round number attached to it. It bites hardest on extinction rates, where the choice of denominator does more work than the sample does, and which are taken apart in the piece on [how an extinction rate is actually derived](/en/insight/extinction-rate-estimates-and-their-assumptions).
