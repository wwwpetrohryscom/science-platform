...
Genotyping arrays interrogate a fixed panel of positions, and the causal variant is usually not one of them. What makes the design work anyway is [linkage disequilibrium](/en/glossary/linkage-disequilibrium): nearby variants are inherited together in blocks, so a genotyped marker acts as a proxy for whatever else sits in its block. The same fact-sheet is explicit that the associated variants may themselves not cause anything, and may simply be tagging along with the real causal variants.
...
An association count is not a count of mechanisms, and the composition of the growth matters. The Catalog's maintainers name the rapid increase in large-scale molecular quantitative-trait studies — publications analysing hundreds to thousands of protein, lipid and metabolite measurements across multiple cohorts, which made up 82 per cent of all studies added to the Catalog in 2023 — as one of the challenges they have had to handle. Those entries are scientifically valuable, bridge genetic associations to mechanism via proteins and metabolites, and are not new disease loci. A million associations in a database is a statement about how much scanning has been done, not about how much has been explained.

## Polygenic scores and the portability failure

A [polygenic score](/en/glossary/polygenic-score) sums a person's trait-increasing alleles, each weighted by its estimated effect, into a single number. The construction is straightforward. The problem is that the weights are estimated in one population and then applied in another.
