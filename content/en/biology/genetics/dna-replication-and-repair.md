...
One comparison makes the point that this accuracy is a deliberate expenditure rather than a physical necessity. Transcription and translation both run at an error frequency near one in 10⁴ — a hundred thousand times the roughly one-in-10⁹ error rate *Molecular Biology of the Cell* gives for the act of copying, which is a different quantity from the per-generation mutation rate above. A mistranslated protein is degraded; a mis-replicated base is inherited by every descendant cell. The cell buys fidelity only where errors are heritable.
...
Lynch syndrome, arising from variants in the mismatch repair genes *MLH1*, *MSH2*, *MSH6* and *PMS2* or in *EPCAM*, produces a different signature: tumours accumulate length changes at repetitive microsatellite tracts. Those single-base insertions and deletions are the errors an intact mismatch-repair system corrects most efficiently of all — better than 99.999 per cent of them, in the case of a single-base deletion in a run of ten G–C pairs — which is exactly why their accumulation is diagnostic of a repair-defective tumour. Population prevalence has been estimated at 1 in 279, and the syndrome accounts for approximately 3 per cent of colorectal and 3 per cent of endometrial cancers. Cumulative colorectal cancer risk to age 70 differs sharply by gene — around 44 per cent in females and 53 per cent in males with *MLH1* variants, against about 3 per cent for *PMS2*.
