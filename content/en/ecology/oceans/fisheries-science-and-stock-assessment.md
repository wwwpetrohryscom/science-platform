...
Assessment output is converted into advice through reference points. In United States federal management, a stock is **subject to overfishing** when its harvest rate exceeds the rate that would produce [maximum sustainable yield](/en/glossary/maximum-sustainable-yield), and **overfished** when its population size has fallen far enough to jeopardise its ability to produce that yield. The two are separate determinations, and a stock can be on one list and not the other.
