...
[Curtailment](/en/glossary/curtailment) — deliberately reducing available output — is usually reported as waste. It is better read as a system declining to pay for energy it cannot use, and its causes are diagnosable. A survey of global solar curtailment published in *Solar Energy* traces it to transmission that cannot move remote output to load, a mismatch between when output peaks and when demand does, and oversupply when variable generation plus inflexible must-run plant exceeds demand — and concludes that the differences between systems reflect policy and grid planning practices as much as geography or season.
...
Aggregating variable output over a wide area reduces its variance, because weather systems are spatially correlated over a limited range and sites far enough apart do not rise and fall together. That makes transmission the least exotic form of flexibility available: it substitutes for storage, reserve and firm capacity at once, without a round-trip loss. It also has the longest lead time, which is why the binding constraint in many systems is now a queue rather than a technology. The International Energy Agency's *Electricity 2026* assessment puts 1,200 to 1,600 GW of advanced-stage projects in connection queues worldwide, and estimates that 450 to 700 GW of them could be released by grid-enhancing technologies on existing lines — dynamic line rating and power-flow control, together with heavier upgrades such as reconductoring and voltage uprating — and a further 750 to 900 GW by more flexible, non-firm connection agreements. Both routes work on corridors that already exist, rather than on new ones.
...
The operating record makes the distinction visible. Capacity factors published by the US Energy Information Administration — read here from the August 2026 release, in which values from 2025 onward are still preliminary — show the utility-scale coal fleet at 52.8 per cent in 2016, 40.5 per cent in 2020 and 48.7 per cent in 2025. Nothing about those boilers changed; relative fuel prices and dispatch order did. Over the same years the nuclear fleet held between roughly 91 and 93 per cent, reflecting a fuel cost so low that running flat out is always the economic choice. One fleet's number tracks the market and the other's tracks maintenance scheduling.
