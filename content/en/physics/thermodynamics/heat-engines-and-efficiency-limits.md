...
The corollary is the part that gets forgotten. Working fluid, cycle geometry, blade design and control strategy do not appear in the expression. They determine how close a machine gets to the ceiling; only the two temperatures determine where the ceiling is. This is why the fuel is nearly irrelevant to the ranking: EIA's 2024 operating table puts the coal fleet at 10,777 Btu per kilowatt-hour and the nuclear fleet at 10,443 — the nuclear entry is a capacity-weighted tested figure rather than an operating one, but the two sit within a few per cent of each other despite having nothing in common but the fact that both raise steam at a temperature far below a combustion flame.
