...
**Maximum-entropy-production heuristics are not laws.** The argument that the atmosphere operates near a maximum-entropy-production state is consistent with observations and useful in coarse modelling, but it is not derivable from first principles in general. Treating it as a law produces overconfident predictions; treating it as a useful approximation does not.
